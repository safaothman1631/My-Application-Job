...
export function Signup({ onSignup }: { onSignup: (user: any) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signupResponse.ok) {
        setSuccess('هەژمار بەسەرکەوتوویی دروستکرا!');
        ...
        setError(signupData.message || 'هەڵەیەک ڕوویدا لە دروستکردنی هەژمار');
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {success && (
        <div className="bg-green-50 border-r-4 border-green-500 p-4 rounded-lg animate-fade-in">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <select
            ...
            className="w-full pl-4 pr-12 py-3.5 border-2 border-gray-200 rounded-xl focus:border-teal-500 focus:ring-4 focus:ring-teal-100 transition-all duration-200 appearance-none cursor-pointer"
          >
            <option value="customer">کاستەمەر</option>
            <option value="provider">پرۆڤایدەر</option>
          </select>
        </div>
      </div>
